...
On remarque qu'en ajoutant les termes d'interaction l'erreur a diminué. Cependant le modèle avec interaction est très complexe (157 termes) donc certainement trop variable. L'idée est de chercher un modèle plus prédictif, i.e. avec une RMSE plus faible que 4.417.
...
```{r}
dim(coef.lars(mod_lasso)) # 157 valeurs de lambda différentes et 78 coefficients
# Faire un choix de ligne entre 1 (très forte pénalité) et 157 (faible pénalité)
l = 1
...
cv_errors <- CV$cv.error
fractions <- CV$index

first_derivative <- diff(cv_errors) / diff(fractions)
second_derivative <- diff(first_derivative) / diff(fractions[-1])

max_second_derivative <- max(abs(second_derivative))
threshold <- 0.1 * max_second_derivative

best_fraction_index <- which(abs(second_derivative) < threshold)[1]

best_fraction <- fractions[best_fraction_index + 1]
best_error <- cv_errors[best_fraction_index + 1]

cat("La première fraction où la dérivée seconde devient inférieure à", threshold, "est :", best_fraction, "\n")
cat("L'erreur de validation croisée à cette fraction est :", best_error, "\n")
...
```{r}
#a completer :
coef_lasso_beter <- predict(mod_lasso, s = value, mode = "fraction", type = "coef")

#a completer :
nb_coef_nul = sum(coef_lasso_beter$coef == 0)
nb_coef_nul
```
...
#a completer :
fits <- predict.lars( object =  mod_lasso,newx=newx, s=value,type='fit', mode='fraction')
#a completer :
...
mod_ridge = lm.ridge(V79 ~.-1,data = housing.train2,lambda = lambda)
...
Construire un modèle de Type Random Forest. - quelle est la différence entre un modèle type bagging et un modèle de forets aléatoires ?
